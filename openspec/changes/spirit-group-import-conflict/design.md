...
- **[風險] 解決衝突選擇「保留 xlsx 值」，但該值是系統未知組名**——寫回後這位學員在分組總表會變成孤兒（不屬於任何 `spirit_ambassador_groups` 記錄的欄位，`page.tsx` 的 `rosterGroupMap` 目前只依 `spirit_ambassador_groups` 產生欄位，這位學員的分組值若不在其中，該學員會遺失在總表視覺呈現之外，只存在於 `students` 表原始資料）。
  → 緩解：`page.tsx` 的既有「資料品質提醒」區塊（`noGroup` 等）可以延伸涵蓋「有分組值但該值不在 `spirit_ambassador_groups` 中」這個新的資料品質類別，讓管理者至少能發現異常、手動新增對應組別或改用其他分組——這個延伸列入 tasks.md，但不改變本次的 spec 核心範圍。
