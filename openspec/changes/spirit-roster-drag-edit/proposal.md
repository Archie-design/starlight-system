...
同時，系統目前不是分組資料的唯一權威來源——關懷長維運的「原官網資料庫」是另一個獨立系統，管理者需要把總表定版後的樣貌匯出成檔案，才能手動比對、更新回原官網。因此匯出功能要能讓管理者清楚看到「目前最終分組結果」全貌，供其手動核對搬移。
...
- 新增「匯出分組總表」功能：管理者可將目前分組總表的最終樣貌（組別、組內排序、組員姓名、小隊長/補課狀態標示）匯出成 CSV 檔案，格式比照 `app/courses/CourseClient.tsx` 既有的 `downloadRosterCsv()` 前端匯出模式（UTF-8 BOM、`Blob`+`<a download>`），供管理者帶去手動比對更新「原官網資料庫」。
