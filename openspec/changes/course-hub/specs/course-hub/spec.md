...
### Requirement: 名單匯出
課程專區內所有學員名單彈窗（階別名單、梯次名單、五運班名單、課後課出席／缺席名單、未全部完課名單、聯誼會已報名名單、二階已完課未報聯誼會名單）SHALL 提供「匯出 CSV」功能，於瀏覽器端直接產生並下載，不經過伺服器 API。匯出內容 SHALL 包含姓名、狀態、備註三欄，並以 UTF-8 BOM 開頭以避免 Excel 開啟中文亂碼；欄位值含逗號、雙引號或換行時 SHALL 正確跳脫。名單為空時匯出按鈕 SHALL 停用。

#### Scenario: 匯出任一名單
- **WHEN** 使用者在任一名單彈窗點選「匯出 CSV」
- **THEN** 瀏覽器直接下載一份 CSV 檔（檔名包含名單標題與當日日期），內容為該名單當前顯示的所有學員（姓名、狀態、備註）
