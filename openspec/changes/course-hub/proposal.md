## Why

現有儀表板（`/dashboard`）已提供全體系彙總的「各階上課人數」與「付款狀態分布」兩張圖表，但只到「體系層級」的粗粒度——無法回答「一階第83梯有幾人、各自完款了嗎」「總共欠了多少錢」「哪些人卡在待確認梯次很久」這類需要進一步鑽研的問題。實際盤點資料庫確認：一階到五階報名人數依序遞減（2738 → 1866 → 1036 → 664 → 591 人，符合漏斗現象），每階都有具體梯次可分（一階67個梯次、二階41個…），且每階都有一定規模的欠款人數（一階405人、二階393人…）。這些梯次與金額層級的資訊目前完全沒有地方可以檢視，需要一個更深入、可互動鑽研的課程專區來補足。

## What Changes

- 新增「課程專區」頁面（`/courses`），與現有的儀表板／資料維護／關懷長專區／心之使者專區／小天使專區並列於頂部導覽。
- 涵蓋範圍：一階至五階（主課程體系）+ 五運班（僅付款統計，不做梯次分析，因其資料無梯次概念）。特殊課程（生命數字系列、生命蛻變、告別負債）不納入本次範圍（欄位只有有/無標記，無狀態/付款資訊，性質與「報名/完款」主題不符）。
- 提供的分析層面：
  - **梯次層級分析**：選定某一階後，顯示該階底下各梯次的人數分布（現有 dashboard 完全沒有這個粒度）。
  - **付款金額細節**：不只顯示欠款人數，也顯示具體欠款金額總和（依階別、可選依梯次）。
  - **選定階別/梯次後檢視學員名單**：比照心之使者專區、小天使專區的既有互動模式，點選某階或某梯次後可看到對應學員名單，並可連結到 `/students` 進一步操作。
  - **課後課完課狀況**（實作過程中經使用者確認後追加）：一至五階的課後課出席資料（xlsx 裡的「一階課程 - 同學會」等 18 個欄位）原本完全沒有被匯入過，`students` 表沒有對應欄位。本次一併新增這些欄位、更新匯入邏輯，並在課程專區顯示每階的完課率（依主課報名者為統計母體），可展開查看各堂課的出席/缺席名單。
  - **聯誼會報名統計**（同上追加）：「聯誼會加入日」「聯誼會組別」原本也沒有被匯入（既有的 `membership_expiry` 是會籍到期日，不是報名狀態）。本次新增這兩個欄位，在課程專區顯示報名人數、未報名人數、組別分布。
  - **未全部完課整體名單**（實作過程中經使用者於真實資料上線後再次確認後追加）：既有的課後課完課狀況僅提供「單堂缺席名單」，無法一次找出「還有課後課沒上完、需要鼓勵」的整體對象。本次在各階（有課後課者）新增一份「未全部完課」名單，與單堂緣席名單分開呈現，每位學員標示已上堂數／總堂數。
  - **二階已完課未報聯誼會交集名單**（同上追加）：新增一份「二階已完課但尚未報名聯誼會」的交集名單，供邀請招募聯誼會使用。「已完課」精確判定為 `course_2` 解析狀態為「已上課」（排除正取／候補／待確認梯次／中離），避免把尚未實際上課的人誤列入招募名單。
  - **名單匯出（CSV）**（同上追加）：課程專區內所有學員名單彈窗（階別、梯次、五運班、課後課出席／缺席、未全部完課、聯誼會已報名、二階已完課未報聯誼會）皆新增「匯出 CSV」按鈕，於瀏覽器端直接產生下載，不新增伺服器 API。匯出內容含 ID、姓名、手機、LINE ID、狀態、備註六欄——手機與 LINE ID 供關懷員後續實際聯繫名單上的學員使用（實作後經使用者確認補上）。
  - **名單表格化＋篩選排序**（同上追加）：所有名單彈窗改為表格呈現（姓名／手機／LINE ID／狀態／備註），提供關鍵字搜尋、狀態下拉篩選，姓名/狀態/備註三欄可點標題排序（狀態欄若為「已上 X / Y 堂」格式依數值排序）。彈窗加寬以容納表格，切換名單時篩選與排序條件會重置。
  - **排除已停開課程（一階「上級貴人成功學」）**（同上追加）：一階完課率統計改為以5堂（原6堂）為母體計算，`l1_makeup_5` 欄位、匯入邏輯、既有歷史出席資料完全保留不動，僅從完課率統計清單移除。
  - **一階解圓夢計劃資格名單**（同上追加）：新增一份資格名單——一階已完課（`course_1` 為「已上課」）且額外上完3堂以上一階課後課（不含同學會）者符合「解圓夢計劃」資格。與課後課完課狀況不同（後者要求全部堂數皆出席），這是固定門檻堂數的資格判定，且明確排除同學會這堂。名單同樣具備既有的表格、篩選、排序、CSV 匯出功能。
- 沿用既有的體系隔離規則（`getEffectiveSystem`），所有統計與圖表僅限使用者有效體系內的資料。

## Capabilities

### New Capabilities
- `course-hub`：課程報名與完款狀況的梯次層級檢視、金額統計、學員名單鑽研。

### Modified Capabilities
（無——不修改任何既有 capability 的既有需求；`/dashboard` 現有的課程相關圖表維持不動，本次純粹新增一個更深入的獨立頁面，不整合、不取代。）

## Impact

- **新頁面**：`app/courses/page.tsx`（Server Component，比照 `app/spirit/page.tsx`／`app/little-angel/page.tsx` 的資料撈取＋體系過濾模式）、`app/courses/CourseClient.tsx`（Client Component，比照既有 hub 頁面的 recharts 圖表呈現模式）。
- **共用工具**：重用既有的 `lib/utils/courseUtils.ts`（`parseCourseValue`、`buildCourseValue`）解析梯次與狀態；重用付款欄位的既有數字判定邏輯（原為 `lib/utils/studentStatus.ts` 內私有的 `NUMERIC` 正規表達式，本次改為 export 供本專區重用）。
- **導覽**：既有 5 個 hub 風格頁面（dashboard、students、maintenance、counselors、spirit、little-angel）新增「課程」連結；新頁面本身也含返回這些頁面的連結。
- **資料庫**（實作過程中追加，見上方 What Changes）：新 migration `018_course_makeup_and_club.sql`，新增 18 個課後課出席欄位（`l1_makeup_1`~`l5_makeup_1`）與 2 個聯誼會欄位（`club_join_date`、`club_group`）。
- **匯入邏輯**：`lib/import/transform.ts`（`DEFAULT_COL`、`HEADER_TO_COL_KEY`、`transformSourceRow()`）、`lib/import/diff.ts`（`COMPARABLE_FIELDS`）、`lib/supabase/types.ts`（`Student`/`StudentInsert`）、`supabase/seed/migrate.ts`（一次性種子腳本，新欄位填 `null`，來源舊總表無對應資料）同步更新，已用真實樣本檔案（`reference/學員資料庫 20260826 (1).xlsx`）驗證匯入正確。
- **未全部完課名單、二階/聯誼會交集名單、CSV 匯出**（實作過程中追加，見上方 What Changes）：`app/courses/page.tsx` 新增 `incompleteMakeupCount`／`incomplete-makeup-{level}` 名單、`L2ClubGap` 計算與 `club-not-joined-l2` 名單；`app/courses/CourseClient.tsx` 新增對應 UI 區塊，以及共用的 `downloadRosterCsv()` 純前端 CSV 匯出函式，掛在名單 Modal 上，涵蓋所有既有與新增的名單類型。不新增伺服器 API、不新增資料庫欄位。`RosterStudent` 額外帶出既有的 `phone`／`line_id` 欄位（不需新 migration，`students` 表本來就有），CSV 匯出六欄為 ID、姓名、手機、LINE ID、狀態、備註。已對照真實資料庫驗證：各階「已全部出席＋未全部完課＝已上主課人數」在 5 階皆精確成立；`course_2` 狀態分布確認嚴格篩選（僅計「已上課」）行為正確；抽樣確認二階/聯誼會交集名單中手機／LINE ID 的實際填寫率（433/435 有手機、425/435 有 LINE ID）；CSV 跳脫、BOM 前綴、新增欄位以獨立測試驗證。
- **名單彈窗表格化＋篩選排序**（同上追加）：`app/courses/CourseClient.tsx` 將名單彈窗從單欄卡片列表改為表格（姓名/手機/LINE ID/狀態/備註），加入 `rosterSearch`／`rosterStatusFilter`／`rosterSort` 三個篩選/排序狀態與 `openRoster()` 包裝函式（開啟新名單時重置這些狀態），狀態欄支援「已上 X / Y 堂」格式的數值感知排序。CSV 匯出改為匯出篩選/排序後的結果。不新增伺服器 API、不新增資料庫欄位。已用獨立測試驗證排序比較器（數值優先、非數值格式退回字串排序）行為正確。
- **排除已停開課程（同上追加）**：`app/courses/page.tsx` 的 `MAKEUP_CLASSES[1]` 移除「上級貴人成功學」（`l1_makeup_5`）這筆項目，一階完課率統計母體從6堂改為5堂。`l1_makeup_5` 資料庫欄位、`lib/import/transform.ts` 的匯入映射、`lib/import/diff.ts` 的 `COMPARABLE_FIELDS`、既有554筆歷史出席資料完全不動——僅從課程專區的完課率計算清單中排除，可隨時復原無需遷移。已對照真實資料庫驗證：一階完課率母體由6堂改為5堂後，「全部堂數皆已出席」人數從75人正確提升為128人（總數皆為2753人，計算一致）。
- **一階解圓夢計劃資格名單（同上追加）**：`app/courses/page.tsx` 的 `MAKEUP_CLASSES` 各堂新增 `isReunion?: boolean` 標記（明確標記同學會，不用 label 字串比對）；新增 `DREAM_PROGRAM_THRESHOLD` 常數與 `L1DreamProgram` 型別，計算一階已完課（`course_1` 精確為「已上課」）且已上非同學會課後課達門檻堂數者，建立 `roster['l1-dream-program']` 名單。`app/courses/CourseClient.tsx` 新增對應資訊卡片，重用既有的表格式名單 Modal（篩選/排序/CSV 匯出全部自動具備，無需額外程式碼）。不新增資料庫欄位、不新增伺服器 API。已對照真實資料庫驗證：一階已完課2169人，非同學會堂數分布 {0:1144, 1:252, 2:242, 3:371, 4:160}（總和吻合），符合資格（≥3堂）531人，與程式計算一致，並抽樣驗證個別學員資料正確。
- **不影響**：`/dashboard` 既有的課程相關圖表與其資料來源、既有課程欄位的顯示與篩選行為、既有的 `students` 表欄位。
